...
describe("inferWindowHeightFromReference", () => {
  ...
  it("applies the manual offset once without changing the entered height", () => {
    const reference = { referenceHeightCm: 10, calibrationOffsetCm: 3 };
    expect(inferWindowHeightFromReference(500, 50, reference.referenceHeightCm, reference.calibrationOffsetCm)).toBe(130);
    expect(inferWindowHeightFromReference(500, 50, reference.referenceHeightCm, reference.calibrationOffsetCm)).toBe(130);
    expect(reference.referenceHeightCm).toBe(10);
    expect(inferWindowHeightFromReference(500, 50, 10)).toBe(100);
    expect(inferWindowHeightFromReference(500, 50, 0, 3)).toBeNull();
  });
});
